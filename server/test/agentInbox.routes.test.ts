...
describe('agent inbox API', () => {
  it('lists threads for the tenant', async () => {
    ...
    const res = await app.inject({ method: 'GET', url: '/api/agent/threads', headers });
  ...
  it('approving a send_reply action queues the email and executes the action', async () => {
    ...
    const res = await app.inject({
      method: 'POST', url: `/api/agent/actions/${action.id}/approve`,
      headers,
    });
    expect(res.statusCode).toBe(200);
  ...
  it('blocks a non-admin from approving', async () => {
    ...
    const res = await app.inject({ method: 'POST', url: `/api/agent/actions/${action.id}/approve`, headers: userHeaders });
