...
export async function upsertThreadsFromReplies(pool: pg.Pool): Promise<number> {
  const r = await pool.query(
    `INSERT INTO conversation_threads (tenant_id, contact_id, campaign_id, latest_inbound_email_id, source, stage)
     ...
     DO UPDATE SET
       ...
       stage = CASE WHEN conversation_threads.stage IN ('converted','lost','closed','unsubscribed')
                    THEN conversation_threads.stage ELSE 'needs_triage' END,
       updated_at = now()
     WHERE conversation_threads.latest_inbound_email_id IS DISTINCT FROM EXCLUDED.latest_inbound_email_id`,
  );
  return r.rowCount ?? 0;
}

export async function getThread(pool: pg.Pool, tenantId: string, id: string): Promise<ThreadRow | null> {
  const r = await pool.query<ThreadRow>(`SELECT ${SELECT} FROM conversation_threads WHERE tenant_id = $1 AND id = $2`, [tenantId, id]);
...
export async function listThreads(
  ...
  const r = await pool.query<ThreadRow>(
    `SELECT ${SELECT} FROM conversation_threads WHERE ${where.join(' AND ')}
...
export async function setThreadAfterSend(pool: pg.Pool, tenantId: string, id: string, outboundEmailId: string): Promise<void> {
  await pool.query(
    `UPDATE conversation_threads SET latest_outbound_email_id=$3, stage='awaiting_customer', status='open', updated_at=now()
     WHERE tenant_id=$1 AND id=$2`,
...
export async function setThreadOwner(pool: pg.Pool, tenantId: string, id: string, ownerUserId: string): Promise<void> {
  await pool.query(`UPDATE conversation_threads SET owner_user_id=$3, updated_at=now() WHERE tenant_id=$1 AND id=$2`, [tenantId, id, ownerUserId]);
}
